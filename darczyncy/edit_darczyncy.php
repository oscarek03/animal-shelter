...
try {
    ...
    $sql = "BEGIN edytuj_darczynce(:id, :nazwa_uzytkownika, :imie, :nazwisko, :mail); END;";
    ...
    if ($result) {
        echo json_encode(['success' => true, 'message' => 'Darczyńca został pomyślnie zaktualizowany.']);
    } else {
        $e = oci_error($stid);
        echo json_encode(['success' => false, 'message' => 'Błąd podczas edycji darczyńcy: ' . $e['message']]);
    }
} catch (Exception $e) {
