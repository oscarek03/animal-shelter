<?php
    ...
    try {
        ...
        $sql =
            "BEGIN DODAJ_DAROWIZNE(:darczynca_id, :kwota, TO_DATE(:data, 'YYYY-MM-DD')); END;";
        ...
        if ($result) {
            echo json_encode([
                "success" => true,
                "message" => "Darowizna została dodana pomyślnie.",
            ]);
        } else {
            $e = oci_error($stid);
            echo json_encode([
                "success" => false,
                "message" => "Błąd podczas dodawania darowizny: " . $e["message"],
            ]);
        }
