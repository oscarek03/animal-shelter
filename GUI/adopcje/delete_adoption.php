...
$get_zwierze_sql =
    "SELECT ZWIERZE_ID FROM REJESTR_ADOPCJI WHERE ID_ADOPCJI = :id";
...
if ($zwierze_row) {
    ...
    $update_status_sql =
        "UPDATE ZWIERZETA SET STATUS = 'Dostepny' WHERE ID = :id_zwierzecia";
    ...
    if (!$update_result) {
        ...
        echo json_encode([
            "success" => false,
            "message" => "Błąd zmiany statusu zwierzęcia: " . $e["message"],
        ]);
    ...
    echo json_encode([
        "success" => false,
        "message" => "Nie znaleziono zwierzęcia związane z tą adopcją.",
    ]);
    oci_free_statement($get_zwierze_stid);
    oci_close($conn);
    exit();
